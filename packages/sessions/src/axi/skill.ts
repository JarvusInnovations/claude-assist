...
/**
 * Generator for the machine-maintained command-reference region of
 * skills/assist-sessions/SKILL.md. The narrative guidance in SKILL.md is
 * hand-authored and lives outside the markers; this produces only the command
 * reference, derived from the same source the CLI uses — so the skill can never
 * drift from the implementation.
 *
 * The bundled CLI is invoked by its path relative to the skill directory
 * (`scripts/sessions-axi`), since the skill may be installed without the binary
 * on PATH.
 */

const SKILL_INVOCATION = "scripts/sessions-axi";
 ...
 * Throws if a declared region's markers are missing so drift can't pass silently.
 */
