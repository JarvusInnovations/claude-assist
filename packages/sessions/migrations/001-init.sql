...
BEGIN
    NEW.search_vector :=
        ...
        setweight(to_tsvector('english', COALESCE(
            (SELECT string_agg(value::text, ' ') FROM jsonb_array_elements_text(NEW.tools_used)), ''
        )), 'B') ||
        setweight(to_tsvector('english', COALESCE(
            (SELECT string_agg(value::text, ' ') FROM jsonb_array_elements_text(NEW.files_touched)), ''
        )), 'B') ||
