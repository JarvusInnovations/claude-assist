 ...
 *
 * Only what the tana-inbox executor needs is implemented: initialize,
...
export class TanaMcpClient {
   ...
   * JSON-RPC, or tool (`isError`) failures — callers (the routing layer)
   * own retries.
   */
  ...
  private async initialize(): Promise<void> {
    const response = await this.request('initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'claude-assist-capture', version: '0.1.0' },
    });
    if (response.error) {
