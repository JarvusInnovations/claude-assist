 ...
 * 80-bit randomness). Duplicated from the capture module rather than
 * shared, so each module stays independently deployable (mirrors capture's
 * own idiom of owning its ULID helper).
 *
...
/**
 * Derive a deterministic ULID from a timestamp + stable seed string. Same
 * (timeMs, seed) always yields the same ULID — used to give meal-bank sheet
 * projections (which are never persisted as rows) a stable identifier
 * across reads.
 */
